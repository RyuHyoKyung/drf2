...
예산 금액을 입력하면, 구당 할당되는 CCTV 카운터를
자동으로 알려주는 AI 프로그램을 작성하시오.
'''
...
class Service(Reader):
    ...
    def save_folium_map(self):
        # saved/police_norm.csv -> police_norm
        # data/kr_states.json
        # data/crime_in_seoul.csv - crime
        # saved/police_pos.csv -> police_pos

        f = self.f
        r = self.r
        p = self.p
        f.context = './data/'
        kr_states = "./data/kr_states.json"
        ...
        folium_map = folium.Map(location=[37.5502, 126.982], zoom_start=12, title='Stamen Toner')  # 위도경도잡기, 뷰사이즈

        folium.Choropleth(
            geo_data=kr_states,  # 지도 그리는 옵션란
            name="choropleth",
